...
export function caching() {
    return gulp.src([
        ...
        // '!' + config.resources + '/views/vendor/mail/**/*.php',
    ])
...
export function copyHTML() {
    return gulp.src(config.dist + '/html/common/index.html', { allowEmpty: true })
...
export function css() {
    return gulp.src(config.app + '/*.css')
        ...
        .pipe(plugins.base64({
            ...
            exclude: [/\.server\.(ru|com|net)\/dynamic\//],
...
export function cssFont() {
    return gulp.src(config.app + '/css/fonts.css', { allowEmpty: true })
        .pipe(plugins.plumber({ errorHandler: onError }))
...
export function imagesOptim() {
    return gulp.src([config.dist + '/images/**/*.{png,gif,jpg,jpeg,svg}',
    config.dist + '/favicon/**/*.{png,gif,jpg,jpeg,svg}',
    '!' + config.dist + '/images/**/no-optimize/*.{png,gif,jpg,jpeg,svg}'])
        // .pipe(plugins.image({
        //     zopflipng: false,
        ...
        .pipe(imagemin([
            imageminPngquant(),
            imageminZopfli(), // дает незначительное улучшение, но сильно нагружает процессор
            imageminMozjpeg(),
            imagemin.svgo({
                plugins: [{
                    removeViewBox: true
...
function convertToWebP() {
    return gulp.src([config.dist + '/images/**/*.{png,jpg,jpeg}',
    config.dist + '/favicon/**/*.{png,jpg,jpeg}',
    '!' + config.dist + '/images/**/no-optimize/*.{png,jpg,jpeg}'], {
            nodir: true
        })
        .pipe(imagemin([
            imageminWebp({
...
export function watcher() {
    gulp.watch([config.app + '/**/*.html'], gulp.series(templateToHTML, htmlHint, copyHTML, reload));
    gulp.watch([config.app + '/**/*.css', '!' + config.app + '/css/fonts.css'], css);
    gulp.watch([config.app + '/css/fonts.css'], gulp.series(cssFont, templateToHTML, copyHTML, reload));
 ...
 * Build
 */
const buildHTML = gulp.series(clearHTMLFolder, templateToHTML, copyHTML, htmlHint);
const buildCSS = gulp.parallel(css, cssFont);
const buildFrontend = gulp.series(prodModeOn, buildCSS, webpackStream, buildHTML, caching, copyHTML);
...
// оптимизация изображений
gulp.task('images', gulp.parallel(imagesOptim, convertToWebP));
...
// генерация favicon
gulp.task('favicon', gulp.series(generateFavicon, injectFaviconMarkups, templateToHTML, copyHTML, caching));
// проверка версии favicon
